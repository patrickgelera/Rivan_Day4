...
conf t
 ...
 int g3
  ...
  exit
 int g4
  ip add 10.22.1.11 255.255.255.0
  no shut
  exit
 !
...
__Make Sure EDGE-22 is Configured__
...
### Task 1: Prevent traffic only from VLAN 100 (CUCM) to reach UTM-PH (10.22.1.11)
~~~
!@UTM-PH
config t
...
### Task 2: Prevent traffic coming from the PC (10.22.1.10) and the EDGE Router from reaching UTM-PH (10.22.1.11)
~~~
!@UTM-PH
config t
...
### Task 3: Allow the PC (10.22.1.10) to access HTTP, SSH, and ICMP of UTM-PH (10.22.1.11)
...
!@UTM-PH
config t
 ip host www.bet11.com 10.22.1.11
 service finger
 ...
 telephony-service
  ...
  max-ephones 5
  max-dn 20
  ip source-address 10.22.1.11 port 2000
  exit
 voice service voip
 ...
 voice register global
  mode cme
  source-address 10.22.1.11 port 5060
  max-dn 12
  max-pool 12
...
- Allow Pings destined for your PC (10.22.1.10)  
...
config t
 ip access-list extended FWP5
  permit  icmp  any  host 10.22.1.10             log 
...
conf t
 ...
 !
 no ip route 10.0.0.0 255.0.0.0 10.22.1.4
 no ip route 200.0.0.0 255.255.255.0 10.22.1.4
 end
~~~
...
| SERVER      | INSIDE PORT | OUTSIDE IP     | OUTSIDE PORT |
| ---         | ---         | ---            | ---          |
| CoreTAAS    | 23          | 200.0.0.22 | 2023         |
| CoreBABA    | 23          | 200.0.0.22 | 4023         |
| CUCM        | 23          | 200.0.0.22 | 8020         |
...
!@EDGE-22
conf t
 ip nat inside source static tcp  10.22.1.4  23  200.0.0.22  4023
 !
 ip nat inside source static tcp  10.22.1.4  __  __.__.__.__     __
 ip nat inside source static tcp  10.22.1.4  __  __.__.__.__     __
 end
~~~
...
!@EDGE-22
conf t
 ip nat inside source static tcp  10.22.1.11  6969  200.0.0.22  3306
 end
~~~
